...
"""
    ...
    def send_email_live(self, to_email, subject, html_content, text_content='', cc_email=''):
        ...
        # 1. First priority: Brevo (Sendinblue) HTTPS API (Free 300 emails/day, pure HTTPS, zero firewall/SSO blocks)
        ...
        if brevo_key:
            try:
                ...
                payload = {
                    'sender': {'name': sender_name, 'email': 'nikhil.shelke@onixnet.com'},
                    'to': [{'email': to_email}],
                    'subject': subject,
                ...
                if resp.status_code in [200, 201]:
                    return True, f"Dispatched via Brevo API from nikhil.shelke@onixnet.com to {to_email}"
                else:
                    return False, f"Brevo API HTTP {resp.status_code}: {resp.text[:120]}"
            except Exception as be:
                return False, f"Brevo API error: {str(be)}"
        ...
        if webhook_url:
            try:
                ...
                elif resp.status_code == 401 or 'onixnet.com/s/' in webhook_url:
                    webhook_err = "Webhook requires Okta SSO login. Please create Webhook in personal @gmail.com or use Brevo API."
                else:
                    webhook_err = f"Webhook HTTP {resp.status_code}: {resp.text[:100]}"
