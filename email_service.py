...
"""
    ...
    def send_email_live(self, to_email, subject, html_content, text_content='', cc_email=''):
        ...
        # 1. First priority: Brevo (Sendinblue) HTTPS API (100% works on Render without firewall or Okta blocks)
        ...
        # 2. Second priority: Google Apps Script Webhook
        ...
        if webhook_url:
            try:
                ...
                with urllib.request.urlopen(req, timeout=15) as resp:
                    ...
                    webhook_err = f"Webhook HTTP {resp.status}"
            except urllib.error.HTTPError as he:
                if he.code == 401 or 'onixnet.com/s/' in webhook_url:
                    webhook_err = "Webhook requires Okta SSO login. Please use Brevo API Key instead."
                else:
                    webhook_err = f"Webhook HTTP {he.code}"
            except Exception as we:
                webhook_err = f"Webhook exception: {str(we)}"

        if not user or not pwd:
            if webhook_err:
                return False, f"{webhook_err} (SMTP fallback credentials not set)"
            return False, "Email Credentials (Brevo API key, Webhook, or SMTP App Password) not configured yet."
