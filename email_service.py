...
class EmailService:
    ...
    def save_smtp_config(self, host=None, port=None, user=None, password=None, sender_name=None, webhook_url=None, brevo_api_key=None):
        ...
        if brevo_api_key is not None: current['brevo_api_key'] = brevo_api_key.strip()
...
"""
    ...
    def send_email_live(self, to_email, subject, html_content, text_content='', cc_email=''):
        ...
        self.smtp_config = self._load_smtp_config()
        user = (self.smtp_config.get('user') or 'nikhil.shelke@onixnet.com').strip()
        ...
        brevo_key = os.environ.get('BREVO_API_KEY', '') or self.smtp_config.get('brevo_api_key', '')
        webhook_url = os.environ.get('GOOGLE_MAIL_WEBHOOK', '') or self.smtp_config.get('webhook_url', '')
        is_render = bool(os.environ.get('RENDER') or os.environ.get('RENDER_SERVICE_ID'))

        # Helper 1: Direct Native Gmail SMTP
        def _try_direct_smtp():
            if not user or not pwd:
                return False, "No SMTP App Password configured."
            try:
                ...
                try:
                    with smtplib.SMTP_SSL(host, 465, timeout=10) as server:
                        ...
                        return True, f"Dispatched natively via Gmail SMTP ({user}) to {to_email}"
        ...
        def _try_brevo():
            ...
            try:
                ...
                payload_dict = {
                    'sender': {'name': sender_name, 'email': user},
                    'to': [{'email': to_email}],
                    'subject': subject,
                    'htmlContent': html_content,
                    'textContent': text_content if text_content else 'Google DRP Enablement Notification. Please view in HTML client.'
                }
                if cc_email and '@' in cc_email:
                    payload_dict['cc'] = [{'email': cc_email}]

                payload_bytes = json.dumps(payload_dict).encode('utf-8')
                req = urllib.request.Request('https://api.brevo.com/v3/smtp/email', data=payload_bytes, headers=headers, method='POST')
                with urllib.request.urlopen(req, timeout=15) as resp:
                    if resp.status in [200, 201]:
                        return True, f"Dispatched via Brevo API from {user} to {to_email}"
                    return False, f"Brevo HTTP {resp.status}"
            except urllib.error.HTTPError as he:
                err_body = he.read().decode('utf-8', errors='ignore')
                return False, f"Brevo API Error ({he.code}): {err_body[:120]}"
            except Exception as be:
                return False, f"Brevo API exception: {str(be)}"

        # Helper 3: Google Mail Webhook
        def _try_webhook():
            if not webhook_url:
            ...
            try:
                ...
                return False, f"Webhook HTTP {he.code}"
            except Exception as we:
                return False, f"Webhook exception: {str(we)}"

        # PRIORITY DISPATCH LOGIC:
        # Local Portal mode -> Direct Native SMTP (first) -> Brevo (fallback) -> Webhook
        # Render Cloud mode -> Brevo API (first) -> Direct SMTP (fallback) -> Webhook
        if not is_render:
            if pwd:
                ok, msg = _try_direct_smtp()
                if ok:
                    return True, msg
            if brevo_key:
                ok, msg = _try_brevo()
        ...
        return False, "All email dispatch methods failed. Please verify SMTP password, Brevo API key, or Webhook configuration."
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #F8FAFC; padding: 24px; margin: 0;">
    ...
    def start_campaign_job(self, tier, employees, custom_template=None, cc_manager=True):
        ...
        if drp_service.df is None or len(drp_service.df) == 0:
            job_id = str(uuid.uuid4())[:8]
            self.jobs[job_id] = {
                'id': job_id, 'tier': tier, 'total': 0, 'processed': 0,
                'success_count': 0, 'failed_count': 0, 'status': 'failed',
                'start_time': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'sent_list': [], 'failed_list': [],
                'error_message': 'No active Excel sheet attached. Email dispatch cancelled for safety.'
            }
            self._save_jobs()
            return job_id

        # Safety filter: ensure recipients only come from active loaded df
        valid_active_emails = set(drp_service.df['email'].fillna('').astype(str).str.strip().str.lower())
        safe_employees = [e for e in employees if e.get('email', '').strip().lower() in valid_active_emails]

        job_id = str(uuid.uuid4())[:8]
        self.jobs[job_id] = {
            'id': job_id,
            'tier': tier,
            'total': len(safe_employees),
            'processed': 0,
            'success_count': 0,
            'failed_count': 0,
            'status': 'running',
        ...
        def _run_worker():
            ...
            if not webhook_url and (not user or not pwd):
                self.jobs[job_id]['status'] = 'failed'
                self.jobs[job_id]['error_message'] = 'SMTP credentials or Google Webhook URL not configured.'
                self._save_jobs()
                return

            job_lock = threading.Lock()

            def dispatch_single_email(emp):
                ...
                try:
                    ...
                    with job_lock:
                        ...
                        if ok:
                            ...
                            self.jobs[job_id]['failed_count'] += 1
                            self.jobs[job_id]['failed_list'].append({'name': name, 'email': to_email, 'reason': msg})
                ...
                with job_lock:
                    if self.jobs[job_id]['processed'] % 5 == 0 or self.jobs[job_id]['processed'] == self.jobs[job_id]['total']:
                        self._save_jobs()

            try:
                # Use multi-threaded executor with 8 parallel workers for high-speed dispatching
                max_workers = min(8, max(1, len(safe_employees)))
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = [executor.submit(dispatch_single_email, emp) for emp in safe_employees]
